...
void suavizar(int ancho, int alto, RGB *imagen)
{
  ...
  if(rank == 0) {
    ...
    for(int i = 1; i < world_size; i++) {
      ...
      MPI_Send(&(imagen[offset]), ancho*chunk_size, rgb_type, i, 1, MPI_COMM_WORLD);
    ...
    MPI_Recv(imagen, ancho*chunk_size, rgb_type, MASTER, 1, MPI_COMM_WORLD, &status);
  }
  ...
  if(rank == MASTER) {
    ...
    for(int i = 1; i < world_size; i++) {
      ...
      MPI_Recv(&(imagen[offset]), ancho * chunk_size, rgb_type, status.MPI_SOURCE, 1, MPI_COMM_WORLD, &status);
    }
    // Suavizar bordes
    /* Esquinas */
    ...
    MPI_Send(imagen, ancho * chunk_size, rgb_type, MASTER, 1, MPI_COMM_WORLD);
  }
}
