...
void suavizar(int ancho, int alto, RGB *imagen)
{
	...
	int chunk = ancho * (alto / world_size);
	int leftover = ancho*(alto%world_size);

	RGB *imagenLocal = malloc(ancho * (alto / world_size) * sizeof(RGB));

	if (rank == 0)
	{
		for (int i = 0; i < world_size; i++)
		{
			MPI_Isend(imagen + leftover + (chunk * i), ancho * (alto / world_size), rgb_type, i, 0, MPI_COMM_WORLD, &requests[i]);
			MPI_Wait(&requests[i], MPI_STATUS_IGNORE);
		}
	}

	MPI_Irecv(imagenLocal, ancho * (alto / world_size), rgb_type, 0, 0, MPI_COMM_WORLD, &req);
	...
	MPI_Isend(imagenLocal, ancho * (alto / world_size), rgb_type, 0, 0, MPI_COMM_WORLD, &req);

	if (rank == 0)
	{
		/* El proceso master recibe las imágenes locales (partes) del resto de procesos */
		for (int i = 0; i < world_size; i++)
		{
			MPI_Irecv(imagen + leftover + (chunk * i), ancho * (alto / world_size), rgb_type, i, 0, MPI_COMM_WORLD, &req);
			MPI_Wait(&req, MPI_STATUS_IGNORE);
		}
		// Suavizar bordes
