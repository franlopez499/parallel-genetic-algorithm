...
{
  MPI_Status status;
  int tagOff = 0, tagPob = 1;

  if(rank == MASTER) {
   *offset += chunk_size + leftover;
   for(int i = 1; i < world_size; i++) {
     MPI_Send(offset, 1, MPI_INT, i, tagOff, MPI_COMM_WORLD);
     MPI_Send(&(pobMaster[*offset]), chunk_size, individuo_type, i, tagPob, MPI_COMM_WORLD);
     *offset += chunk_size;
   }
  } else {
   MPI_Recv(offset, 1, MPI_INT, MASTER, tagOff, MPI_COMM_WORLD, &status);
   MPI_Recv(pobRecv, chunk_size, individuo_type, MASTER, tagPob, MPI_COMM_WORLD, &status);
  }
}
...
void gatherPoblacion(int *offset, int chunk_size, Individuo *pobMaster, Individuo *pobSend)
{
  ...
  if(rank == MASTER) {
    for(int i = 1; i < world_size; i++) {
      MPI_Recv(offset, 1, MPI_INT, MPI_ANY_SOURCE, tagOff, MPI_COMM_WORLD, &status);
      MPI_Recv(&(pobMaster[*offset]), chunk_size, individuo_type, status.MPI_SOURCE, tagPob, MPI_COMM_WORLD, &status);
    }
  } else {
      MPI_Send(offset, 1, MPI_INT, MASTER, tagOff, MPI_COMM_WORLD);
      MPI_Send(pobSend, chunk_size, individuo_type, MASTER, tagPob, MPI_COMM_WORLD);
  }
}
...
void crear_imagen(const RGB *imagen_objetivo, int num_pixels, int ancho, int alto, int max, int num_generaciones, int tam_poblacion, RGB *imagen_resultado, const char *output_file)
{
  ...
  if(rank == MASTER) {
    ...
    // Se envía la población ordenada para evolucionarla
    offset = 0;
    scatterPoblacion(&offset, chunk_size, leftover, poblacion, NULL);
    chunk_size += leftover;
  } else {
    ...
    // Recibe la población ordenada para su evolución
    scatterPoblacion(&offset, chunk_size, leftover, NULL, poblacion);
  }
